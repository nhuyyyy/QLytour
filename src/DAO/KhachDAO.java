	...
	public int sua(Khach khach) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,"Lỗi sửa Database");
                ...
                public List<Khach> getKhach(Map<String,String> nv)throws Exception{
               List<Khach> ds = new ArrayList<Khach>();
              MyDataAccess my = new MyDataAccess("localhost","root","","qltour");
		try {
			StringBuffer query = new StringBuffer("Select * from khach where 1=1");
                        if(nv.get("Makhachhang")!= null && !nv.get("Makhachhang").equals("")){
                            query.append(" and makhach = "+nv.get("Makhachhang"));
                        }
                        if(nv.get("tenkhachhang")!= null&& !nv.get("tenkhachhang").equals("")){
                            query.append(" and tenkh ="+nv.get("tenkhachhang"));
